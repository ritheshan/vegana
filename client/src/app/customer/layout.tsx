...
import {
  Compass, User, Calendar, Heart, MessageSquare, Settings,
  LogOut, ShieldAlert, Sparkles, ChevronRight
} from 'lucide-react';
...
export default function CustomerDashboardLayout({
  ...
  React.useEffect(() => {
    ...
    if (!isAuthenticated) {
      router.push('/auth/login?redirect=/customer');
    } else if (role !== 'customer') {
      // Allow developer bypass, but if it is actually in organizer/admin, route appropriately
      // router.push(`/${role}`);
    }
  }, [isAuthenticated, role, router]);

  const navLinks = [
    { label: 'Dashboard Hub', path: '/customer', icon: User },
    { label: 'My Bookings', path: '/customer/bookings', icon: Calendar },
    { label: 'Wishlist Toggles', path: '/customer/wishlist', icon: Heart },
    { label: 'Custom Requests', path: '/customer/custom-requests', icon: Sparkles },
    { label: 'Profile Settings', path: '/customer/settings', icon: Settings },
  ];

  const getActiveLabel = () => {
    const active = navLinks.find(link => pathname === link.path);
    return active ? active.label : 'Customer Portal';
  };
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6">
        <div className="flex flex-col items-center gap-3 text-center">
          ...
          <span className="text-xs text-slate-400 font-bold">Authenticating session...</span>
        </div>
      </div>
    );
  }
        ...
        <div className="flex flex-col gap-8">
          ...
          <div className="flex items-center gap-3 px-2 py-3 bg-slate-900 border border-slate-800 rounded-xl">
            ...
            <div className="flex flex-col min-w-0">
              ...
              <span className="text-[9px] font-bold text-teal-400 uppercase tracking-widest leading-none mt-0.5">Explorer</span>
            </div>
          </div>
      ...
      <div className="flex-1 flex flex-col min-w-0 max-h-screen overflow-y-auto">
        ...
        <header className="hidden md:flex items-center justify-between px-8 h-16 bg-white border-b border-slate-200">
          <div className="flex items-center gap-2">
            <span className="text-xs text-slate-400 font-bold uppercase tracking-widest">Customer Portal</span>
          ...
          <div className="flex items-center gap-4">
            <Badge variant="accent" className="font-extrabold py-0.5">Explore Vibe Match: 98%</Badge>
          </div>
        </header>
